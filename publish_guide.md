...
### HTMLビューとJSONデータの完全分離について
本プロジェクトは、HTML（カレンダーの見た目や操作を管理するビュー）と、カレンダーに表示する予定データ本体（`docs/events.json`）を完全に切り離して構築しています。
これにより、将来的に以下のような方法でデータ更新の自動化・システム化を簡単に行うことができます。
1. **外部システム（スプレッドシートや管理画面など）** からカレンダーのイベント日程を更新する。
2. それに基づき、自動的に `docs/events.json` だけを出力・書き換える。
3. HTMLファイル（`index.html`）をビルド（再生成）することなく、Web上のカレンダーデータが自動的に最新化されます。
